...
const binaryGapSolution = (num: number): number => {
...
const rotateArraySolution = (arr: number[], k: number): number[] => {
  const n = arr.length;
  const rotations = k % n;
...
const rotateArraySolutionRecursively = (arr: number[], k: number): number[] => {
  if (k === 0) return arr;
...
const rotateArr = (arr: number[]): number[] => {
...
const oddOccurencesSolution = (arr: number[]): number => {
...
const binaryGapSolution2 = (num: number): number => {
  const binary = num.toString(2);
  const zeros = [];
  let current = '';
...
const rotateArrSolution = (arr: number[], k: number): number[] => {
  if (k === 0) return arr;
...
const rotateArr2 = (arr: number[]): number[] => {
  const pop = arr.pop();
